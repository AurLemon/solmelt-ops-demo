...
4. 检查 `git branch --show-current`、`git status --short`、`git remote -v`。如果工作区为空且需要吸收组长对版本、根提示词或公共文档的更新，Agent 直接在当前业务分支执行 `git pull --no-rebase origin main`；如果有本地改动、分支不对或出现冲突，必须停止并先向成员报告，不能丢弃改动或强行覆盖。
