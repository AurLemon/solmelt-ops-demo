...
3. 执行 `pnpm env:check`；首次启动按 README 配置 `.env`、数据库与 Seed。
4. 检查 `git branch --show-current`、`git status`、`git remote -v`；有远端时执行 `git fetch origin`，确认当前分支未落后于对应远端分支。
...
- 工作前先 `git pull origin <branch>`；若 push 被拒绝或怀疑漏拉，依次检查 `git status`、`git remote -v`、`git fetch origin`、`git log HEAD..origin/<branch>`，先保存或提交本地工作后再合并远端变更。
- 修改 `shared/`、Prisma、迁移、根配置、依赖、Docker、公共布局或 tooling 前，必须提交 Contract Change；不得用越界修改逃避作用域检查。
- 完成后删除调试代码，执行 `pnpm format` 与 `pnpm pr:check`，提交完整 diff 给新的只读 AI 会话 Review；成员本人必须能够口述数据流和关键实现。
