...
6. 涉及 UI 时阅读 docs/ui-guidelines.md
7. 本 PR 的完整 diff
...
- 菜单路径、页面路由、权限码、Token 存储、API 字段和数据源是否与公共契约一致；是否用 Layer 临时配置或重复实现公共能力兜底。
- 涉及 UI 时，是否覆盖暗色/浅色、桌面/移动端、loading/empty/error/ready，以及从侧栏进行的客户端导航；不要把刷新后可用当成导航可用。
...
AI Review 不能替代组员负责。Review 后由成员确认：数据从哪里进入、写入哪些表、发布什么事件、页面如何消费、失败时如何表现。跨领域 PR 还要由组长在合并后的 `main` 复走真实用户路径；无法口述或无法复现时不得宣布集成完成。
