...
先检查 git status、git remote -v、当前分支是否落后 origin/feat/auth。再复述：任务目标、最低验收、允许修改范围、涉及的 API、准备修改的文件、人工验收动作。未确认前不要扩大范围。
...
完成后删除调试日志，执行 pnpm format 与 pnpm pr:check，说明数据流、实际修改文件、验证结果和未实现的可选项，再交给新的只读 AI 会话 Review。
```
