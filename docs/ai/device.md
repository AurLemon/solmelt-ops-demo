...
先检查 git status、git remote -v、当前分支是否落后 origin/feat/device，再复述任务、最低验收、文件和验收动作。使用 Nuxt 4、Nuxt UI、TypeScript Interface、Zod、Prisma 和 ApiResult。Service 只写 Device 领域表，成功后通过 EventBus 发事件。
...
完成后执行 pnpm format 与 pnpm pr:check，并说明设备状态、在线状态和最新数据的来源、实际修改文件与验证结果，再交给新的只读 AI 会话 Review。
```
