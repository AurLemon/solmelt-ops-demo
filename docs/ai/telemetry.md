...
上报接口严格使用 TelemetryReportInput，验证设备存在、未删除、已启用、时间合法、属性存在且类型匹配。报告、属性值、最新值和三项阈值报警必须在同一 Prisma transaction 内完成；成功提交后发布 telemetry.reported 仅供后续审计/通知扩展，Handler 不得回写这些核心数据。三项超限均创建 WARNING、UNHANDLED 报警。
