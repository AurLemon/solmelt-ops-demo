...
按任务书要求，每 10 至 30 秒向 /api/v1/telemetry/report 发送 9 台设备的
随机游走数据，偶发注入明确标注的异常值，用于驱动 65°C / 4.5 mm/s / 60 A
三项报警闭环。

使用方式：
    python3 scripts/simulator/simulate.py
    python3 scripts/simulator/simulate.py --url http://127.0.0.1:3000 --interval 15
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="固定发送间隔（秒）；省略则在 10-30 秒间随机",
    )
    parser.add_argument(
        "--abnormal-rate",
        type=float,
        default=0.05,
        help="单次发送触发异常注入的概率，默认 0.05",
    )
    parser.add_argument(
        "--timeout",
        type=float,
...
def main() -> int:
    ...
    print(
        ...
        f"interval={'固定 ' + str(args.interval) + 's' if args.interval else '随机 10-30s'} "
    ...
    while running:
        ...
        if not running:
            break
        delay = args.interval if args.interval is not None else random.uniform(10, 30)
